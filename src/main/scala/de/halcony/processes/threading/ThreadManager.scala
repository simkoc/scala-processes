...
class ThreadManager[T] extends LogSupport {
  ...
  protected[threading] def setLambda(lambda : T => Unit) = {
  ...
  protected[threading] def encounteredError(input : Option[T], thr : Throwable) : Unit = {
  ...
  protected[threading] def setThreadCount(count: Int): ThreadManager[T] = synchronized {
    threadCount = count
    this
  }
  ...
  private val threads : collection.mutable.Map[Int,Thread] = collection.mutable.Map()
  private val threadsJob : collection.mutable.Map[Int,Option[T]] = collection.mutable.Map()
            ...
            while(parentManager.getKeepRunning) { // as long as the threads are supposed to run
              parentManager.synchronized { // sync with the parent
                ...
                case Some(job) => try { // if there was a job
                  ...
                  case thr: Throwable => parentManager.encounteredError(Some(job), thr)
                } finally {
  ...
  def stop(gracePeriodMs : Long = 100): Boolean = {
    ...
    this.threads.values.map { // go through all threads
      thread =>
        val delta = List((start + gracePeriodMs) - System.currentTimeMillis(), 0).max // get remaining grace period
  ...
  def waitFor(timeoutMs : Long): Boolean = {
    val start = System.currentTimeMillis() // get the current time
    this.synchronized {
      var delta = (start + timeoutMs) - System.currentTimeMillis() // calculate the remaining time
      while (areThreadsAlive() && delta > 0) { // while there are still threads alive and time to wait
        delta = (start + timeoutMs) - System.currentTimeMillis() // calculate the remaining time
